...
The current expression of system's internal entropy (the `Seed`) gets
...
distribution, de-correlating (in distribution sense) its lower and higher
32-bit parts. The lower 32 bits of the `Seed` are then updated by a mix of its
own higher 32 bits, the hash word produced on previous rounds, and the
...
of internal and hash word's entropy. Yet, the message "shifts" the system into
a new state, predictated by previous messages. Thus, it is possible to "give
names" to random number generators: for example, pass a word "Michelle" to
the hashing function, and then the generation will continue in the space
predictated by this initial word.
