...
* **Information loss**
  $G$ maps a 192-bit input to a 64-bit output. For a random mapping, every $Y$ has about $2^{128}$ preimages.  
  The attacker can at best recover *a* preimage, not the original unique state.

* **Mixed arithmetic / Boolean non-linearity**
  $G$ combines addition modulo $2^{64}$, multiplication modulo $2^{64}$, XOR, and bit-rotation.  
  The unknowns $X_i, L_i, H_i$ appear inside multiplications, XORs and additions, creating a system of 64 Boolean equations with 192 unknowns.  
  The algebraic degree is very high due to carry propagation and modular multiplication, and no sub-exponential solution method is known for such mixed systems over $\mathbb{Z}_{2^{64}}$.

* **No analytical shortcut**
  Even after guessing two of the three state variables, the remaining equation in the third is an entangled mix of operations that does not admit direct inversion.  
  The best generic approach is enumeration: guessing 128 bits of the state reduces the search to $O(2^{64})$ evaluations, and a full 192-bit brute force costs $O(2^{192})$.

* **Exponential lower bound**
  The state space size is $2^{192}$, the output size $2^{64}$. On average, finding a preimage requires at least $2^{128}$ trial evaluations if $G$ behaves like a random function.  
  $2^{128}$ is exponential in the security parameter (the word size of 64 bits), so the transformation is computationally irreversible.
